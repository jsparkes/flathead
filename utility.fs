...
// Use Map<int, int> in F#
//module IntMap = Map.Make(struct type t = int let compare = compare end)

// Make functions with OCaml names
let (mod) a b = a % b
let (land) a b = a &&& b
let lnot n = ~~~ n
let (lor) a b = a ||| b
...
// We're not using the byte type to be more consistent
// with the OCaml code.
...
let bytes_to_string (buf : byte[]) = System.Text.ASCIIEncoding.ASCII.GetString(buf)
...
let reflexive_closure_many items relation =
  ...
  List.fold_left (fun s i -> if Set.contains i s then s else Set.add i s) t items
